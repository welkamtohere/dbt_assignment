...
categories as (
    ...
    join {{ ref('stg_categories') }} c on c.category_id = fc.category_id
    group by fc.film_id
),
...
rentals as (
    ...
    left join {{ ref('stg_rentals') }} r on r.inventory_id = i.inventory_id
    group by i.film_id
)
...
left join {{ ref('rating_descriptions') }} rd on rd.rating = f.rating
